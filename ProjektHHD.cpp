...
void bestellung:: zeigListe(void) {//Liste zeigen
	cout << nr<<"            "<<zahl_essen[0]<<"*"<<name1<<",  "<< zahl_essen[1]<<"*"<<name2<<",  "<<zahl_essen[2]<<"*"<<name3<<",  ";
	cout <<zahl_essen[3]<<"*"<<name4<<",  "<<zahl_essen[4]<<"*"<<name5<<",  ";
	if(zustand==1){cout << "Bestellt";}
	else if(zustand==2){cout << "In Bearbeitung";}
	else if(zustand==0) {cout << "Abholbar";}
	cout << endl;
	}
...
double preis_fkt (int Auswahl, int menge) 
{
	if(Auswahl ==1) {
			menge1+=menge;
			preis=essen1.preisAus()*menge;}
		else if(Auswahl ==2){
			menge2+=menge;
			preis=essen2.preisAus()*menge;}
		else if(Auswahl ==3) {
			menge3+=menge;
			preis=essen3.preisAus()*menge;}
		else if(Auswahl ==4) {
			menge4+=menge;
			preis=essen4.preisAus()*menge;}
		else if(Auswahl ==5) {
			menge5+=menge;
			preis=essen5.preisAus()*menge;	}
			Gesamtpreis += preis;
			return Gesamtpreis;
		}
...
int main() {														// Anfang des Hauptprogramms
	...
	for( int i=0; i<5; i++) {// Bestellung Schleife läuft hier 5 mal
		...
		cout <<fixed<<setprecision(2)<<"4. "<< essen4.preisAus()<<" Euro  "<<essen4.nameAus()<< endl;
		cout <<fixed<<setprecision(2)<<"5. "<< essen5.preisAus()<<" Euro  "<<essen5.nameAus()<< endl;
		cout <<"6. Beenden"<<endl;
	...
	//KOCH
	for(int i=0; i<5; i++) {
		Bestellung[i].name1=essen1.nameAus();
		Bestellung[i].name2=essen2.nameAus();
		Bestellung[i].name3=essen3.nameAus();
		Bestellung[i].name4=essen4.nameAus();
		Bestellung[i].name5=essen5.nameAus();
	}
